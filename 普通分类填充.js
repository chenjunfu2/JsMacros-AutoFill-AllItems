...
function getItemId(pos, faceOffset) {
    ...
    if (frameItem) {
        ...
        if (nbt) {
            extraNbt = `,tag:${nbt.toString().replace(/^NBTElementHelper:/, "").slice(1, -1)}`;
        }
...
function buildFunnelCommands(funnelPos, itemData) {
    ...
    if (maxCount === 16) {
        ...
        cmds.push(`/data modify block ${x} ${y} ${z} Items[3] set value {Slot:3b,id:"minecraft:iron_nugget",Count:4b,tag:{RepairCost:0,display:{Name:'{"text":"乐色B"}'}}}`);
        cmds.push(`/data modify block ${x} ${y} ${z} Items[4] set value {Slot:4b,id:"minecraft:iron_nugget",Count:4b,tag:{RepairCost:0,display:{Name:'{"text":"乐色B"}'}}}`);
    } else {
        // 64 堆叠 → slot0 放 2 个
